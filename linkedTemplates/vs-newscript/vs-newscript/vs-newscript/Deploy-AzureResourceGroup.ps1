...
# This PowerShell script requires the Azure PowerShell Az modules
# To install PowerShell, see https://learn.microsoft.com/powershell/scripting/install/installing-powershell-on-windows
# To install Azure PowerShell Az, see https://learn.microsoft.com/powershell/azure/install-azps-windows
...
try {
    [Microsoft.Azure.Common.Authentication.AzureSession]::ClientFactory.AddUserAgent("VSAzureTools-$UI$($host.name)".replace(' ','_'), '17.7.0')
} catch { }
...
Write-Host "Using PowerShell $($PSVersionTable.PSVersion.Major).$($PSVersionTable.PSVersion.Minor).$($PSVersionTable.PSVersion.Patch) from $PSHome"
