...
param sourceWindowsAdminUserName string = 'username'
...
param targetSqlDbAdministratorLogin string = 'username'
...
resource sourceServerName 'Microsoft.Compute/virtualMachines@2017-12-01' = {
  ...
  properties: {
    ...
    diagnosticsProfile: {
      bootDiagnostics: {
        enabled: true
        storageUri: reference(storageAccountName_var, '2017-10-01').primaryEndpoints.blob
      }
    }
  }
}

resource sourceServerName_SqlIaasExtension 'Microsoft.Compute/virtualMachines/extensions@2017-12-01' = {
  name: '${sourceServerName.name}/SqlIaasExtension'
  location: location
  properties: {
    type: 'SqlIaaSAgent'
...
resource sourceServerName_CustomScriptExtension 'Microsoft.Compute/virtualMachines/extensions@2017-12-01' = {
  name: '${sourceServerName.name}/CustomScriptExtension'
  location: location
  properties: {
...
resource DMSServiceName 'Microsoft.DataMigration/services@2017-11-15-privatepreview' = {
  sku: {
...
resource DMSServiceName_SqlToSqlDbMigrationProject 'Microsoft.DataMigration/services/projects@2017-11-15-privatepreview' = {
  name: '${DMSServiceName.name}/SqlToSqlDbMigrationProject'
  location: location
  properties: {
    sourcePlatform: 'SQL'
...
resource sourceNicName 'Microsoft.Network/networkInterfaces@2018-02-01' = {
...
resource sourceServerNSG 'Microsoft.Network/networkSecurityGroups@2018-02-01' = {
...
resource publicIPSourceServer 'Microsoft.Network/publicIPAddresses@2018-02-01' = if (createPublicIP) {
  sku: {
    name: 'Basic'
    tier: 'Regional'
  }
...
resource adVNet 'Microsoft.Network/virtualNetworks@2018-02-01' = {
  name: adVNet_var
  location: location
  properties: {
    addressSpace: {
...
resource adVNet_defaultSubnetName 'Microsoft.Network/virtualNetworks/subnets@2018-02-01' = {
  name: '${adVNet.name}/${defaultSubnetName}'
  properties: {
    addressPrefix: '10.2.0.0/24'
  }
}
...
resource targetServerName 'Microsoft.Sql/servers@2015-05-01-preview' = {
...
resource targetServerName_databaseName 'Microsoft.Sql/servers/databases@2017-10-01-preview' = {
  sku: {
    name: 'Basic'
    tier: 'Basic'
  }
  ...
  dependsOn: [
    resourceId('Microsoft.Sql/servers', concat(targetServerName_var))
  ]
}

resource targetServerName_databaseName_Import 'Microsoft.Sql/servers/databases/extensions@2014-04-01-preview' = {
  name: '${targetServerName_databaseName.name}/Import'
  properties: {
  ...
  dependsOn: [
    resourceId('Microsoft.Sql/servers/databases', targetServerName_var, databaseName)
  ]
}

resource targetServerName_AllowAllWindowsAzureIps 'Microsoft.Sql/servers/firewallrules@2014-04-01-preview' = {
